			...
			if (midi_param_count >= message_size && !midi_in_sysex)
			{
				if (channels[channel].midi_enabled)
				{
					...
					case 0xB:
						...
						{
							//printf("[Sound] unhandled message CONTROL CHANGE %02X %02X", midi_param_bytes[0], midi_param_bytes[1]);
						}
						break;
					case 0xC:
...
LoopySound::LoopySound(std::vector<uint8_t> &rom_in, float out_rate, int buffer_size)
{
	...
	//printf("[Sound] Init uPD937 core: synth rate %.01f, out rate %.01f, buffer size %d", synth_rate, out_rate, buffer_size);
...
bool LoopySound::enqueue_midi_byte(char midi_byte, int timestamp)
{
	...
	{
		if (!midi_overflowed)
			//printf("[Sound] MIDI queue overflow, increase queue capacity or send smaller groups more often.");
